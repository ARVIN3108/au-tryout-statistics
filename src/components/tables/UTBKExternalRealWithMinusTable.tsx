...
export default function UTBKExternalRealWithMinusTable({
  data,
}: {
  data: Row[];
}) {
    ...
    <table className="text-center text-sm text-gray-500 opacity-90 rtl:text-right dark:text-gray-400">
      <thead className="border-b-2 bg-gray-50 text-xs whitespace-nowrap text-gray-700 uppercase dark:bg-gray-800 dark:text-gray-300">
        <tr>
          ...
          <th scope="col" className="p-2.5">
            <span className="absolute -mt-0.5 -ml-4.5"></span>PM
            <br />
            SLH
          </th>
          <th scope="col" className="p-2.5">
          ...
          data.map((row, rowIndex) => (
            ...
            >
              ...
              {row.map((cell, cellIndex) => {
                if (cellIndex == 1 || cellIndex == 24)
                  return (
                    <td
                      key={cellIndex}
                      className={`px-6 py-4 ${cellIndex == 24 && rowIndex == data.length - 1 && "rounded-br-lg"}`}
                    >
                      {cell as string}
                    </td>
                  );
                ...
                if (cellIndex >= 3 && cellIndex <= 24)
                  return (
                    <td key={cellIndex} className="w-2.75 p-2.75">
