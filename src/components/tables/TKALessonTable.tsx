...
export default function TKALessonTable({ data }: { data: Row[] }) {
  return (
    <table className="w-full text-center text-sm text-gray-500 opacity-90 rtl:text-right dark:text-gray-400">
      <thead className="border-b-2 bg-gray-50 text-xs whitespace-nowrap text-gray-700 uppercase dark:bg-gray-800 dark:text-gray-300">
        <tr>
          ...
          <th scope="col" className="p-4">
            <span className="absolute -mt-0.5 -ml-1">INDONESIA</span>
            <br />
            BNR
          </th>
          <th scope="col" className="p-4">
          ...
          <th scope="col" className="p-4">
            <span className="absolute -mt-0.5 -ml-1">MAT WAJIB</span>
            <br />
            BNR
          </th>
          <th scope="col" className="p-4">
          ...
          <th scope="col" className="p-4">
            <span className="absolute -mt-0.5 -ml-5.5">MAPEL PILIHAN 1</span>
            <br />
            BNR
          </th>
          <th scope="col" className="p-4">
          ...
          <th scope="col" className="p-4">
            <span className="absolute -mt-0.5 -ml-5.5">MAPEL PILIHAN 2</span>
            <br />
            BNR
          </th>
          <th scope="col" className="p-4">
              ...
              {row.map((cell, cellIndex) => {
                if (cellIndex == 1 || cellIndex == 15)
                  return (
                    <td
                      key={cellIndex}
                      className={`px-6 py-4 ${cellIndex == 15 && `text-gray-900 dark:text-white ${rowIndex == data.length - 1 && "rounded-br-lg"}`}`}
                    >
                      {cell as string}
                    </td>
                  );
                ...
                if (cellIndex >= 3 && cellIndex <= 14)
                  return (
                    <td key={cellIndex} className="w-2 p-4.5">
